...
- `openspec/specs/<capability>/spec.md` purpose text, `### Requirement:` headings, and `#### Scenario:` headings.
...
Every node ID should be generated from the selected store identity plus explicit OpenSpec identity parts, such as active change name, archive directory, spec capability, requirement heading, and scenario heading. Source file paths should be stored as paths relative to the selected OpenSpec store repository or OpenSpec root so repeated runs from different absolute workspace roots remain as stable as possible.
...
- Archive directory names may include dates while active change names do not -> Mitigation: model active and archived change identity separately and store archive state explicitly.
