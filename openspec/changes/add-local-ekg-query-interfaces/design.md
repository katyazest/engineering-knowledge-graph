...
### Decision: Use FactMCP only as thin wrapper surface

Create a FactMCP-facing module, for example `engineering_kg.mcp.factmcp_server`, that registers query operations as FactMCP tools or resources and delegates immediately to `engineering_kg.query`.
...
Rationale: the user wants FactMCP as the MCP wrapper surface, but the Engineering KG must remain usable by non-MCP callers and tests.
...
- TODO: Decide whether FactMCP wrappers should expose queries as tools, resources, or both.
